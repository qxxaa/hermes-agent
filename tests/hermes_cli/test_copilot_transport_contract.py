...
def test_selected_model_controls_all_credential_paths(
    ...
    tokens = {
        "explicit": "gho_thisisafaketokenfortesting_explicit",
        "normal": "gho_thisisafaketokenfortesting",
        "pool": "gho_thisisafaketokenfortesting_pool",
    }
    kwargs = {"requested": "copilot", "target_model": target}
    pool = None
    if credential_path == "explicit":
        kwargs["explicit_api_key"] = tokens["explicit"]
    elif credential_path == "pool":
        entry = PooledCredential.from_dict("copilot", {
            "access_token": tokens["pool"], "base_url": "https://api.githubcopilot.com",
        })
        pool = SimpleNamespace(provider="copilot", has_credentials=lambda: True, select=lambda **_: entry)
    ...
    assert result["api_key"] == tokens[credential_path]
    if pool is not None:
...
def test_real_sdk_sends_copilot_messages_with_bearer_only(monkeypatch, base_url):
    ...
    with build_anthropic_client("gho_thisisafaketokenfortesting", base_url=base_url) as client:
    ...
    assert request.headers["authorization"] == "Bearer gho_thisisafaketokenfortesting"
