...
def _isolated_copilot_state(monkeypatch):
    ...
    # Copilot is the selected provider, so the pool exchanges rather than seeding the raw token.
    monkeypatch.setattr(hermes_auth, "is_provider_explicitly_configured", lambda provider: provider == "copilot")
    yield
    ac.shutdown_cached_clients()
    copilot_auth._jwt_cache.clear()
    copilot_auth._exchange_failure_cache.clear()
...
def test_spent_copilot_allowance_does_not_block_other_provider_refresh():
    ...
    route = ac._LadderRoute(
        ...
        None, None, timeout=None, full_auth_used=True,
    )
...
def test_failed_exchange_keeps_raw_fallback_for_oauth_token(monkeypatch):
    """Only ghu_ tokens are withheld when the exchange fails; a gho_ token still goes raw."""
    raw_oauth = "gho_synthetic_auxiliary_test"
    monkeypatch.setattr(copilot_auth, "resolve_copilot_token", lambda: (raw_oauth, "synthetic"))
    monkeypatch.setattr(copilot_auth.time, "time", lambda: 1000.0)
    boundary = _Boundary([200])
    monkeypatch.setattr(ac, "_openai_http_client_kwargs", boundary.http_client_kwargs)
    ...
    assert result.choices[0].message.content == "ok"
    assert boundary.authorisations == [f"Bearer {raw_oauth}"]
    assert boundary.requests[-1]["url"].startswith(COPILOT_URL)
