...
def test_timestamped_turn_recovers_clean_canonical_history_after_provider_failure(tmp_path, monkeypatch):
    ...
    try:
        ...
        # The failed turn is closed by a durable assistant boundary, so the resumed
        # input is its own turn. Each logical ask is sent once with its own original
        # timestamp; the failed one is never restamped with the resume time.
        wire = captured[0]["input"]
        assert [item.get("role") for item in wire] == ["user", "assistant", "user"]
        assert wire[0]["content"] == "[Thu 2026-08-20 12:00:00 UTC] failed question"
        assert wire[2]["content"] == "[Thu 2026-08-20 12:00:30 UTC] resume question"
        wire_text = "".join(str(item.get("content")) for item in wire)
        assert wire_text.count("failed question") == 1
        assert wire_text.count("resume question") == 1
        # Durable history stays clean: timestamps are a wire projection only.
        contents = [message["content"] for message in recovered["messages"]]
        assert contents[0] == "failed question"
        assert recovered["messages"][1]["role"] == "assistant"
        assert contents[2:] == ["resume question", "recovered"]
    finally:
        reopened.close()
