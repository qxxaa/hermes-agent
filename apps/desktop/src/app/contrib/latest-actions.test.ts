...
describe('latestActions adapters', () => {
  it('dereferences the latest busy-submit action without falling back to redirect', async () => {
    ...
    actions.onBusySubmit = previous
    const adapted = latestChatActions(actions)
    actions.onBusySubmit = current
    await expect(adapted.onBusySubmit!('unchanged text')).resolves.toBe(true)
    expect(current).toHaveBeenCalledExactlyOnceWith('unchanged text')
